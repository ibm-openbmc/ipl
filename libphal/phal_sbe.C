...
{
    std::array<const uint32_t, 5> cfamAddr = {0x2986, 0x2809, 0x282A, 0x2829,
                                              0x1007};
    for (const uint32_t addr : cfamAddr)
    {
        try
        {
            ...
            if (val != 0xFFFFFFFF)
            {
                log(level::INFO, "%s CFAM(0x%X) : 0x%X", pdbg_target_path(proc),
                    addr, val);
            }
        }
        catch (...)
...
void validateSBEState(struct pdbg_target* chip)
{
    ...
    {
        log(level::INFO, "SBE (%s) is not ready for chip-op: state(0x%08x)",
...
void setState(struct pdbg_target* chip, enum sbe_state state)
{
    ...
    if (rc)
    {
        log(level::ERROR, "Failed to set SBE state(%d) information (%s)", state,
                          ...
                          bool coSuccess)
{
    ...
    while (offset < ffdcLen)
    {
        ...
        {
            log(level::ERROR, "UNKNOWN magic word: (%X) in the FFDC packet",
        ...
        if (iter == ffdcFileList.end())
        {
            ...
            tmpfile_t ffdcFile(const_cast<uint8_t*>(bufPtr + offset),
                               lenInBytes);
            ffdcFileList[slid] =
                std::make_tuple(severity, ffdcFile.getFd(), ffdcFile.getPath());
        }
        else
        {
            // Existing SLID, update severity if higher, append data
            // to existing file
            auto& [existingSeverity, existingFd, existingPath] = iter->second;
            if (severity > existingSeverity)
            {
                existingSeverity = severity; // Update severity if higher
            }
            // Append new data to the existing file
            int fd = open(existingPath.c_str(), O_WRONLY | O_APPEND);
            if (fd == -1)
            {
            ...
            {
                close(fd);
                throw std::runtime_error("Failed to append data to FFDC file");
            }
            close(fd);
        }
...
sbeError_t captureFFDC(struct pdbg_target* target, bool coSuccess)
{
    ...
    else
    {
        if (pdbg_target_probe(get_ody_chipop_target(opTarget)) !=
            PDBG_TARGET_ENABLED)
        {
            log(level::ERROR, "Failed to capture FFDC, Target not enabled (%s)",
    ...
    uint16_t magicByte = ntohs(*reinterpret_cast<uint16_t*>(bufPtr.getData()));
    if (magicByte == ffdcMagicCode)
    {
        // Process as PROC FFDC
        tmpfile_t ffdcFile(bufPtr.getData(), ffdcLen);
        return coSuccess ? sbeError_t(exception::SBE_INTERNAL_FFDC_DATA,
...
sbeError_t capturePfutFFDC(struct pdbg_target* target,
                           std::vector<FFDCEntry>& lientry)
{
    pdbg_target* opTarget = getPibTarget(target);
    bufPtr_t bufPtr;
    uint32_t ffdcLen = 0;
    uint32_t status = 0;
    ...
    if (sbe_ffdc_get(opTarget, &status, bufPtr.getPtr(), &ffdcLen))
    {
        log(level::ERROR, "Failed to get SBE FFDC data (%s)",
            pdbg_target_path(opTarget));
        return sbeError_t(exception::SBE_FFDC_GET_FAILED);
    }
    ...
    while (offset < ffdcLen)
    {
        uint16_t magic_bytes =
            ntohs(*reinterpret_cast<uint16_t*>(bufPtr.getData() + offset));
        FFDCEntry entry;
        uint16_t lenInWords = 0;
        if (magic_bytes == ffdcMagicCode)
        {
            const p10FfdcHeader* hdr = reinterpret_cast<const p10FfdcHeader*>(
                bufPtr.getData() + offset);
            lenInWords = ntohs(hdr->lengthinWords);
            uint32_t fapiRc = ntohl(hdr->fapiRc);
            // this skipping was done in phosphor-logging sbe_handler.C
            constexpr size_t headerWords =
                sizeof(p10FfdcHeader) / sizeof(uint32_t);
            const size_t dataWords = lenInWords - headerWords;

            entry.fapiRc = fapiRc;
            entry.lenInWords = dataWords;
            log(level::INFO, "capturePfutFFDC p10 magic byte 0x%04x \n",
                magic_bytes);
            log(level::INFO, "capturePfutFFDC p10 len in words 0x%04x \n",
                lenInWords);
            log(level::INFO, "capturePfutFFDC p10 fapiRc 0x%08x \n", fapiRc);
            const uint8_t* dataStart =
                bufPtr.getData() + sizeof(p10FfdcHeader) + offset;
            const uint8_t* dataEnd =
                dataStart + (dataWords * sizeof(uint32_t)) + offset;
            entry.data = std::vector<uint8_t>(dataStart, dataEnd);
        }
        else if (magic_bytes == pozFfdcMagicCode)
        {
            const pozFfdcHeader* hdr = reinterpret_cast<const pozFfdcHeader*>(
                bufPtr.getData() + offset);
            lenInWords = ntohs(hdr->lengthinWords);
            uint32_t fapiRc = ntohl(hdr->fapiRc);
            // this skipping was done in phosphor-logging sbe_handler.C
            constexpr size_t headerWords =
                sizeof(pozFfdcHeader) / sizeof(uint32_t);
            const size_t dataWords = lenInWords - headerWords;
            entry.fapiRc = fapiRc;
            entry.lenInWords = dataWords;
            log(level::INFO, "capturePfutFFDC poz magic byte 0x%04x \n",
                magic_bytes);
            log(level::INFO, "capturePfutFFDC poz len in words 0x%04x \n",
                lenInWords);
            log(level::INFO, "capturePfutFFDC poz fapiRc 0x%08x \n", fapiRc);
            const uint8_t* dataStart =
                bufPtr.getData() + sizeof(pozFfdcHeader) + offset;
            const uint8_t* dataEnd =
                dataStart + (dataWords * sizeof(uint32_t)) + offset;
            entry.data = std::vector<uint8_t>(dataStart, dataEnd);
        }
        else if ((ffdcLen - offset) <
                 (minFFDCPackageWordSizePOZ * sizeof(uint32_t)))
        {
            log(level::ERROR, "Reached end of ffdc data skipping bytes %d",
                (ffdcLen - offset));
            break;
        }
        else
        ...
        offset += (lenInWords * sizeof(uint32_t));
        lientry.push_back(entry);
    } // end while
