...
static void ipl_pre0(void) { ipl_pre(); }

static int ipl_poweron(void) { return -1; }

static int ipl_startipl(void) { return -1; }

static int ipl_DisableAttns(void) { return -1; }
...
static int update_hwas_state_callback(struct pdbg_target *target, void *priv) {
  ...
  if (ipl_type() == IPL_TYPE_MPIPL &&
    ...
    if (!set_or_clear_state(target, target_info->set_hwas_state)) {
      ipl_log(IPL_ERROR, "Failed to update functional state of fru type 0x%x\n",
              type);
      // Unable to update the functional state of HWAS attribute of the
      // target, so we need to stop
      return GUARD_TGT_NOT_FOUND;
...
static void process_guard_records() {
  ...
  if (records.size()) {
    ...
    for (const auto &elem : records) {
      ...
      if (!ipl_guard() && !openpower::guard::isEphemeralType(elem.errType)) {
  ...
  pdbg_for_each_class_target("proc", proc) {
    ...
    for (const char *data : mProcChild) {
      pdbg_for_each_target(data, proc, child) {
        ...
        if (!pdbg_target_get_attribute_packed(child, "ATTR_HWAS_STATE", "41", 1,
                                              buf)) {
...
static bool update_genesis_hwas_state(void) {
    ...
    for (const char *data : mProcChild) {
      pdbg_for_each_target(data, proc, child) {
        if (!set_or_clear_state(child, target_enabled)) {
          ipl_log(IPL_ERROR, "Failed to set HWAS state of %s, index %d\n", data,
  ...
  pdbg_for_each_class_target("oscrefclk", clock_target) {
    if (!pdbg_target_get_attribute(clock_target, "ATTR_POSITION", 2, 1,
                                   &clk_pos)) {
    ...
    if (!set_or_clear_state(clock_target, true)) {
      ipl_log(IPL_ERROR, "Failed to set HWAS state of oscrefclk, %s\n",
              pdbg_target_path(clock_target));
      return false;
    }
  }
...
static int ipl_alignment_check(void) { return -1; }
...
static int initialize_and_check_clock_chip(uint8_t &clock_count) {
  ...
  pdbg_for_each_class_target("oscrefclk", clock_target) {
    ...
    if (!skip_reset) {
      ...
      data = OSC_RESET_CMD;
      i2c_rc =
          i2c_write(clock_target, 0, OSC_CTL_OFFSET, OSC_RESET_CMD_LEN, &data);
      if (i2c_rc) {
        ipl_log(IPL_ERROR,
                "soft reset command is failed for clock '%s' with rc = %d\n",
      ...
      std::this_thread::sleep_for(
          std::chrono::milliseconds(CLOCK_RESTART_DELAY_IN_MS));
    }
    ...
    i2c_rc =
        i2c_read(clock_target, 0, OSC_STAT_OFFSET, OSC_STAT_REG_LEN, &data);
    if (i2c_rc) {
      ipl_log(IPL_ERROR,
              "status register read is failed for clock '%s', with rc = %d\n",
              pdbg_target_path(clock_target), i2c_rc);
...
static int ipl_set_ref_clock(void) {
  ...
  pdbg_for_each_class_target("proc", proc) {
    ...
    ipl_log(IPL_INFO,
            "Running p10_setup_ref_clock HWP on master processor %d\n",
            pdbg_target_index(proc));
...
static int ipl_proc_prep_ipl(void) { return -1; }

static int ipl_edmarepair(void) { return -1; }

static int ipl_asset_protection(void) { return -1; }
...
static int ipl_hb_config_update(void) { return -1; }
...
static int ipl_startPRD(void) { return -1; }
