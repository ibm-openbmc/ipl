                 ...
                 size_t endOffset, FFDCMap& ffdc)
{
    ...
    while (offset + sizeof(sbei::pozFfdcHeader) <= endOffset)
    {
        ...
        {
            logger::error(
                "Invalid FFDC magic or length: magic=0x{:04x}, len={}", magic,
                lengthWords);
            return EPROTO;
        }
        ...
        {
            logger::error(
                "FFDC package overruns buffer: offset={}, size={}, buflen={}",
        ...
        ffdc[slid].emplace_back(sbei::FFDCEntry{.data = std::move(ffdcData),
                                                .fapiRc = fapiRc,
                                                .severity = severity});
